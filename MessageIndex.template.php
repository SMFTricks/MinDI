	...
	<div class="tborder topic_table" id="messageindex">
		...
		if (!empty($context['topics']))
		{
			echo '
					...
					<th scope="col" width="10%" class="lefttext hidden-xs"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=replies', $context['sort_by'] == 'replies' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['replies'], $context['sort_by'] == 'replies' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a> / <a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=views', $context['sort_by'] == 'views' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['views'], $context['sort_by'] == 'views' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></th>';
			// Show a "select all" box for quick moderation?
			if (empty($context['can_quick_mod']))
				echo '
					<th scope="col" class="lefttext last_th hidden-xs" width="22%"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></th>';
			else
				echo '
					<th scope="col" class="lefttext hidden-xs" width="22%"><a href="', $scripturl, '?board=', $context['current_board'], '.', $context['start'], ';sort=last_post', $context['sort_by'] == 'last_post' && $context['sort_direction'] == 'up' ? ';desc' : '', '">', $txt['last_post'], $context['sort_by'] == 'last_post' ? ' <img src="' . $settings['images_url'] . '/sort_' . $context['sort_direction'] . '.gif" alt="" />' : '', '</a></th>';
		...
		// No topics.... just say, "sorry bub".
			...
			echo '
					...
					<th colspan="4"><strong>', $txt['msg_alert_none'], '</strong></th>
		...
		foreach ($context['topics'] as $topic)
		{
			...
			if(!empty($settings['separate_sticky_locked']))
			{
					...
					echo '
						<tr class="titlebg">
							<td colspan="', empty($options['display_quick_mod']) ? '7' : '8', '">
								<strong>', $txt['sh_this_fixed'], '</strong>
							</td>
						</tr>
					';
					$stickybar = true;
				...
				elseif (!$topic['is_sticky'] && $stickybar && !$normalbar)
				{
					echo '
						<tr class="titlebg">
							<td colspan="', empty($options['display_quick_mod']) ? '7' : '8', '">
								<strong>', $txt['sh_this_normal'], '</strong>
							</td>
						</tr>
					';
					$normalbar = true;
			...
			echo '
				<tr>
					<td class="status ', $color_class, '">';
						if ($topic['is_sticky'] || ($topic['is_sticky'] && $topic['is_locked']))
						echo '
						<img src="', $settings['images_url'], '/md/sticky.png" alt="" />';
						elseif ($topic['is_locked'])
						echo '
						<img src="', $settings['images_url'], '/md/locked.png" alt="" />';
						elseif ($topic['new'] && $context['user']['is_logged'])
						echo '
						<img src="', $settings['images_url'], '/on.png" alt="" />';
						else
						echo '
						<img src="', $settings['images_url'], '/off.png" alt="" />';
					echo '
					</td>
					<td class="icon1 ', $color_class, ' hidden-xs">
						<img src="', $settings['images_url'], '/topic/', $topic['class'], '.gif" alt="" />
					</td>
					<td class="icon2 ', $color_class, ' hidden-xs">
					...
					<td class="subject ', $alternate_class, '">
						...
						echo '
							', $topic['is_sticky'] ? '<strong>' : '', '<span class="on" id="msg_' . $topic['first_post']['id'] . '">', $topic['first_post']['link'], (!$context['can_approve_posts'] && !$topic['approved'] ? '&nbsp;<em>(' . $txt['awaiting_approval'] . ')</em>' : ''), '</span>', $topic['is_sticky'] ? '</strong>' : '';
						
						else	
						echo '
							', $topic['is_sticky'] ? '<strong>' : '', '<span class="off" id="msg_' . $topic['first_post']['id'] . '">', $topic['first_post']['link'], (!$context['can_approve_posts'] && !$topic['approved'] ? '&nbsp;<em>(' . $txt['awaiting_approval'] . ')</em>' : ''), '</span>', $topic['is_sticky'] ? '</strong>' : '';
			...
			echo '
					...
					<td class="stats ', $color_class, ' hidden-xs">
						', $topic['replies'], ' ', $txt['replies'], '
						<br />
						', $topic['views'], ' ', $txt['views'], '
					</td>
					<td class="lastpost ', $alternate_class, '">
						<a href="', $topic['last_post']['href'], '"><img class="lastpost" src="', $settings['images_url'], '/icons/last_post.gif" alt="', $txt['last_post'], '" title="', $txt['last_post'], '" /></a>
						<img class="mindi_opacity" src="'. $settings['images_url']. '/md/time.png" alt="" /> ', $topic['last_post']['time'], '<br />
						<img class="mindi_opacity" src="'. $settings['images_url']. '/md/user.png" alt="" /> ', $topic['last_post']['member']['link'], '
					</td>';
