...
<form class="flow_hidden" action="', $scripturl, '?action=pm;sa=pmactions;', $context['display_mode'] == 2 ? 'conversation;' : '', 'f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', '" method="post" accept-charset="', $context['character_set'], '" name="pmFolder">';
	...
	<div class="', $window_class, ' clear">
		<div class="date_post"><div>', $txt['from'], ' ', ($context['from_or_to'] == 'from' ? $message['member']['link'] : (empty($message['recipients']['to']) ? '' : implode(', ', $message['recipients']['to']))), ' ', $txt['on'], ' ', $message['time'], '</div><span>', $message['subject'], '</span></div>
		...
		<div class="poster">
			...
			if (!$message['member']['is_guest'])
			{
				...
				if (!empty($modSettings['onlineEnable']) && !$message['member']['is_guest'])
					echo '
						<span class="user_status floatright"><img src="', $message['member']['online']['image_href'], '" alt="', $message['member']['online']['text'], '" /></span>';
		...
		<div class="postarea">
			...
			<div class="post">
				...
				<div class="smalltext reportlinks">
					', (!empty($modSettings['enableReportPM']) && $context['folder'] != 'sent' ? '<div class="righttext"><a href="' . $scripturl . '?action=pm;sa=report;l=' . $context['current_label_id'] . ';pmsg=' . $message['id'] . '"><img src="' .$settings['images_url'] . '/report.png" alt="' . $txt['pm_report_to_admin'] . '" title="' . $txt['pm_report_to_admin'] . '" /></a></div>' : '');
